...
docker compose --env-file "$PROJECT_DIR/.env" run --rm josh-cli onboard --no-install-daemon || {
    echo "  ⚠  Josh onboarding had issues. You may need to run it manually."
    echo "     Try: cd agents/josh && docker compose --env-file ../../.env run --rm josh-cli onboard --no-install-daemon"
}

echo ""
echo "  --- Onboarding Audrey ---"
cd "$PROJECT_DIR/agents/audrey"
docker compose --env-file "$PROJECT_DIR/.env" run --rm audrey-cli onboard --no-install-daemon || {
    echo "  ⚠  Audrey onboarding had issues. You may need to run it manually."
    echo "     Try: cd agents/audrey && docker compose --env-file ../../.env run --rm audrey-cli onboard --no-install-daemon"
}
